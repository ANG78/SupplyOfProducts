  ...
  handle(request: HttpRequest<any>): Observable<HttpEvent<any>> {
    return new Observable((responseObserver: Observer<HttpResponse<any>>) => {
      ...
      switch (request.method) {
        case 'GET':
          if (request.urlWithParams.indexOf('workers') >= 0 ) {
            const workers = this.workers;
            responseOptions = {
              body: {workes: JSON.parse(JSON.stringify(workers))},
              status: 200
            };
          } else if (request.urlWithParams.indexOf('mediaitems?medium=') >= 0 || request.url === 'mediaitems') {
            let medium;
